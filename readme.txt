...
* WP-CLI policy apply: `wp handl-aicac policy apply <file.json> [--dry-run] [--yes] [--allow-mismatched-site]` previews a secret-safe diff, then writes through the same save path as Rules import (restore snapshot kept).
