...
* Dashboard Getting started list after Quick setup: plugins that used AI, a starter policy pack, alert email, the policy tester, and the weekly digest. Items check off from saved settings, hide when done or dismissed, and return if a setting is turned off.
...
* WP-CLI policy apply: preview a secret-safe policy diff with --dry-run, then apply the reviewed JSON export with --yes. A site mismatch requires explicit confirmation, and the previous policy is kept as a restore snapshot.
