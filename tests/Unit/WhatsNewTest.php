...
final class WhatsNewTest extends TestCase {
	...
	public function test_fresh_install_seeds_without_announcement(): void {
		...
		$announce = Whats_New::detect_version_bump( '1.4.0', true );
		$this->assertSame( '', $announce );
		$this->assertSame( '1.4.0', get_option( Whats_New::OPTION_KEY ) );
	...
	public function test_upgrade_to_new_version_announces_once_per_user(): void {
		...
		update_option( Whats_New::OPTION_KEY, '1.3.0', false );

		$announce = Whats_New::detect_version_bump( '1.4.0', false );
		$this->assertSame( '1.4.0', $announce );
		$this->assertSame( '1.4.0', get_option( Whats_New::OPTION_KEY ) );
		$this->assertTrue( Whats_New::should_show_notice_for_user( 7 ) );

		Whats_New::dismiss_for_user( 7, '1.4.0' );
		$this->assertFalse( Whats_New::should_show_notice_for_user( 7 ) );
	...
	public function test_same_version_does_not_reannounce(): void {
		update_option( Whats_New::OPTION_KEY, '1.4.0', false );
		update_option( Whats_New::ANNOUNCE_OPTION_KEY, '1.4.0', false );
		Whats_New::dismiss_for_user( 7, '1.4.0' );
	...
	public function test_highlights_fallback_empty_for_unknown_version(): void {
		...
		$this->assertNotEmpty( Whats_New::highlights_for_version( '1.4.0' ) );
		$this->assertLessThanOrEqual( 5, count( Whats_New::highlights_for_version( '1.4.0' ) ) );
	}

	public function test_opening_panel_dismisses_for_user(): void {
		update_option( Whats_New::OPTION_KEY, '1.3.0', false );
		Whats_New::detect_version_bump( '1.4.0', false );
		$this->assertTrue( Whats_New::should_show_notice_for_user( 7 ) );
		Whats_New::dismiss_for_user( 7, '1.4.0' );
		$this->assertSame( '1.4.0', Whats_New::get_user_dismissed_version( 7 ) );
		$this->assertFalse( Whats_New::should_show_notice_for_user( 7 ) );
	}
}
